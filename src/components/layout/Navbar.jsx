...
export function Navbar() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 text-[var(--theme-text)]">
      <Container as="nav" aria-label="Navegación principal" className="flex min-h-[4.75rem] items-center justify-between lg:min-h-24">
        <a
          href="#inicio"
        ...
        {hasNavigation ? (
          <div className="hidden items-center gap-8 lg:flex">
            ...
            {cta ? (
              <Button className="gap-2.5 px-5 text-[0.68rem] tracking-[0.1em]" href={cta.href} external={cta.external}>
                <span>{cta.label}</span>
        ...
        {hasNavigation ? (
          <button
            ...
            className="flex size-11 items-center justify-center rounded-[var(--theme-radius)] border border-[var(--theme-border)] bg-[var(--theme-surface)] focus-visible:outline-[3px] focus-visible:outline-offset-2 focus-visible:outline-[var(--theme-accent)] lg:hidden"
      ...
      {hasNavigation ? (
        <div
          ...
          ref={menuRef}
        >
          <Container className="flex min-h-[calc(100svh-4.75rem)] flex-col items-start gap-5 py-8">
            {links.map((link) => (
              <a
